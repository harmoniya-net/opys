import { z } from 'zod';
import { type Ruleset, RuleSchema, OsNameSchema } from '@opys/core';
...
export interface Library {
  ...
  /** Mojang OS/feature rules — the shared `@opys/mojang-rules` format. */
...
export function parseLibraries(raws: unknown[]): Library[] {
  ...
  for (const item of raws) {
    ...
    for (const [osName, classifierKey] of Object.entries(raw.natives)) {
      ...
      // uses the `{arch}` placeholder, and opys targets 64-bit exclusively.
