...
import { AssetManifest } from './client/assets';
import { Client } from './client/client';
import { Library } from './client/libraries';
import { VersionManifest, type Version } from './version';

function libraryToUnifact(lib: Library): Unifact {
  const path = `\${library_directory}/${lib.artifact.path}`;

  // Native libraries get extracted into ${natives_directory}
...
/**
 * Build a Unifest from a parsed Minecraft client manifest.
 * Port of the Rust `client()` template function.
 */
export async function minecraftTemplate(client: Client): Promise<Unifest> {
  // Fetch asset object list
  const assetRes = await fetch(client.assetIndex.url);
  if (!assetRes.ok)
    throw new Error(
      `Failed to fetch asset manifest: HTTP ${assetRes.status} ${assetRes.url}`,
    );
  ...
  // Asset objects (content-addressed; skip integrity to avoid hashing thousands of files)
  ...
  // Build a per-OS classpath: all libs whose rules satisfy that OS
  ...
  const vars = new ValDefs([
    ...
    // Per-OS classpath (includes all applicable libs for each platform)
    ...classpathEntries,
  ]);

  // --- launch ---
  // JVM args  + main class  + game args
...
/**
 * Fetch the Mojang version manifest and return the parsed Client for the given version.
 * Pass `undefined` to get the latest release.
 */
