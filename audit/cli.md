...
- **`lib/args.ts` (whole file) — the wrapper earns little over `node:util`.**
  ...
  `extractGlobals` in `opys.ts` already bypasses the wrapper for the global
...
- **`bin/opys.ts:104-118` — the fallback error handler prints two stacks**
  (`err.cause.stack` then `err.stack`), each gated on `OPYS_QUIET`. Verbose;
