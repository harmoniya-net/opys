...
previously the bulk of this package now live in the `opys-core` Rust
...
- `fetch.ts` (build-time HTTP retry) intentionally stays in `@opys/core`
  rather than moving to `runtime` — the consumers (mojang/forge/java/
  curseforge plugins) can't depend on `@opys/runtime` per the
