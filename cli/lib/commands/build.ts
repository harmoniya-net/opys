import { writeFile } from 'node:fs/promises';
import { dirname, resolve } from 'node:path';
import {
  Unifest,
  ValDefs,
  resolveConfig,
  type ArtifactIterable,
  type Unifact,
} from '@unifest/core';
import { parseArgs } from '../args';
import { UsageError } from '../errors';
import { importConfig } from '../fs';
import type { Logger } from '../logger';

async function collectArtifacts(
  sources: ArtifactIterable[],
): Promise<Unifact[]> {
  const out: Unifact[] = [];
  for (const src of sources) for await (const a of src) out.push(a);
  return out;
}

export async function cmdBuild(argv: string[], logger: Logger): Promise<void> {
  const args = parseArgs(argv, [
    { long: 'input', short: 'i', type: 'string' },
    { long: 'output', short: 'o', type: 'string' },
  ]);

  const inputFile = args.getString('input') ?? 'unifest.config.mjs';
  const outputFile = args.getString('output');

  const absConfigFile = resolve(inputFile);
  const configDir = dirname(absConfigFile);
  const mod = await importConfig(absConfigFile);

  if (!mod.default)
    throw new UsageError(`${inputFile}: no default export found`);
  const config = await resolveConfig(mod.default, { mode: 'build' });

  logger.info('Building manifest...');
  const allArtifacts = await collectArtifacts(config.artifacts ?? []);
  logger.debug(`Collected ${allArtifacts.length} artifacts`);
  const rawVars = config.vars;
  const vars =
    rawVars instanceof ValDefs ? rawVars : ValDefs.CODEC.decode(rawVars ?? {});
  const unifest = new Unifest(vars, config.command, allArtifacts);
  const json = JSON.stringify(Unifest.CODEC.encode(unifest), null, 2) + '\n';

  const out = outputFile ?? config.output;
  if (out) {
    await writeFile(resolve(configDir, out), json);
    logger.info(`Written to ${out}`);
  } else {
    process.stdout.write(json);
  }
}
