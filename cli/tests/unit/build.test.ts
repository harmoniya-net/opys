...
beforeEach(async () => {
  dir = await mkdtemp(join(tmpdir(), 'torba-build-'));
});
 ...
 * `mod.default` to be a `TorbaConfigInput`.
 */
...
describe('cmdBuild', () => {
  ...
  it('prints the manifest to stdout when no output is configured', async () => {
    ...
    await writeConfig('torba.config.mjs', noOutput);
    const out: string[] = [];
  ...
  it('passes the mode through to a config function', async () => {
    ...
    await writeConfig('torba.config.mjs', fnConfig);
    await cmdBuild(
