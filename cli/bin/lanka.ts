...
import { NetworkError, IntegrityError, ExtractionError } from '@lanka/runtime';
import { VersionFetchError } from '@lanka/mojang';
import { UsageError } from '../lib/errors';
import { Logger, parseLogLevel, type LogLevel } from '../lib/logger';

const USAGE = `\
lanka — declarative manifest toolkit

USAGE
  lanka build  [-i <lanka.config.mjs>] [-o <out>] [--mode <m>]  Build manifest
  lanka launch [-i <lanka.config.mjs>] [--mode <m>]  Build, install, launch

OPTIONS
  -i, --input          Config file  (default: lanka.config.mjs)
...
main().catch((err) => {
  ...
  // can pinpoint lanka bugs vs config issues without needing to re-run
  // under a debugger. Set LANKA_QUIET=1 to suppress the stack.
  const msg = err instanceof Error ? err.message : String(err);
  process.stderr.write(`Unexpected error: ${msg}\n`);
  if (err instanceof Error && err.cause instanceof Error) {
    process.stderr.write(`  caused by: ${err.cause.message}\n`);
    if (err.cause.stack && !process.env.LANKA_QUIET) {
      process.stderr.write(`${err.cause.stack}\n`);
    }
  }
  if (err instanceof Error && err.stack && !process.env.LANKA_QUIET) {
