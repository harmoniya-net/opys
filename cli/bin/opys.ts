...
main().catch((err) => {
  ...
  // under a debugger. Set OPYS_QUIET=1 to suppress the stack.
  const msg = err instanceof Error ? err.message : String(err);
  process.stderr.write(`Unexpected error: ${msg}\n`);
  if (err instanceof Error && err.cause instanceof Error) {
    process.stderr.write(`  caused by: ${err.cause.message}\n`);
    if (err.cause.stack && !process.env.OPYS_QUIET) {
      process.stderr.write(`${err.cause.stack}\n`);
    }
  }
  if (err instanceof Error && err.stack && !process.env.OPYS_QUIET) {
