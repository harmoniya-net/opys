...
```sh
bun add @unifest/forge @unifest/core zod
```
...
template.artifacts; // Vanilla MC artifacts (Unifact[])
template.vars; // Merged ValDefs (vanilla + Forge classpath)
template.command; // Launch config using Forge's main class
```

The returned `artifacts` cover only vanilla Minecraft JARs. Forge library JARs are expected to be provided separately (e.g. via `artifactScanner` from `@unifest/mc` pointed at your local Forge installation).

## How it works

1. Fetches the vanilla `Client` for the given Minecraft version via `@unifest/mc`.
...
- Pass the resulting `template.vars` and `template.command` directly to your Unifest manifest.
