/**
 * The JS half of `@opys/neoforge`: the typed surface and the plugin closure.
 *
 * Loader behaviour — resolving a build, the document fold, the classpath
 * order, the merged args — is the `opys-neoforge` crate's, and is tested there
  ...
  libraries: [
    {
      ...
      downloads: {
        artifact: {
          ...
          sha1: SHA('a'),
          size: 2000,
          url: 'https://example.invalid/loader.jar',
        },
      },
    },
  ],
...
function route(target: string): unknown {
  ...
  if (target === '/assets/17.json') return ASSET_MANIFEST;
...
describe('resolveNeoForgeVersion', () => {
  it('resolves a bare Minecraft version to its best build', async () => {
    const r = await resolveNeoForgeVersion(MC, base);
    expect(r).toEqual({
      minecraft: MC,
      neoforge: BUILD,
  ...
  it('resolves a build id that shares no text with its Minecraft version', async () => {
    const r = await resolveNeoForgeVersion(BUILD, base);
    expect(r.minecraft).toBe(MC);
    expect(r.neoforge).toBe(BUILD);
  });

  it('rejects a version the index does not list, naming it', async () => {
    await expect(resolveNeoForgeVersion('9.9.9', base)).rejects.toThrow(
      /9\.9\.9/,
    );
  });
});
...
describe('resolveNeoForge', () => {
  ...
  it('hands back values the manifest types can read directly', async () => {
    ...
    const loader = t.artifacts.find((a) => a.path.includes('loader-4.0.39'))!;
    expect(loader.integrity).toEqual({ sha1: SHA('a') });
  });

  it('walks the index, then the document, then the Mojang chain', async () => {
    await resolveNeoForge(options());
  ...
  it('surfaces a failed fetch as a rejection naming the status', async () => {
    await expect(
      resolveNeoForge({ ...options(), source: `${base}/nope` }),
