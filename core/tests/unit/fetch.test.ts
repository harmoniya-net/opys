...
import { fetchWithRetry, OPYS_USER_AGENT } from '../../lib/fetch';
...
describe('OPYS_USER_AGENT', () => {
  it('is a non-empty identifying string', () => {
    expect(OPYS_USER_AGENT).toMatch(/^opys\//);
  });
});
...
describe('fetchWithRetry', () => {
  ...
  it('defaults the User-Agent header', async () => {
    ...
    expect(new Headers(init.headers).get('user-agent')).toBe(OPYS_USER_AGENT);
  });
