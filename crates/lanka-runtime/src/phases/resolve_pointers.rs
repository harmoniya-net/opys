...
async fn fetch_descriptor(url: &str) -> Result<lanka_core::PointerDescriptor, InstallError> {
...
) -> Result<(Source, Option<lanka_core::Integrity>, Option<u64>), InstallError> {
    let mut current = source;
    let mut integrity: Option<lanka_core::Integrity> = None;
