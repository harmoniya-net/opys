...
use crate::fetch::{client, fetch_with_retry, RetryOptions, TORBA_USER_AGENT};
...
async fn discover(
    ...
    if need_head {
        let res = client()
            .head(artifact_url)
            .header("user-agent", TORBA_USER_AGENT)
            .send()
            .await
...
pub async fn resolve_discovery(
    ...
    for artifact in manifest.artifacts {
        ...
        let next = Artifact {
            integrity: integrity.clone().map(torba_core::Integrity::One).or_else(|| artifact.integrity.clone()),
