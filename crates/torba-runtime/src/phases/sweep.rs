...
use tokio::fs;
use torba_core::{glob_base, glob_to_regex, interpolate};
...
fn is_torba_internal(abs: &str) -> bool {
...
/// isn't in `managed` and isn't a torba bookkeeping marker.
async fn sweep_subtree(
    ...
    while let Some(dir) = stack.pop() {
        ...
        while let Some(entry) = rd.next_entry().await? {
            ...
            if is_torba_internal(&abs_str) || managed.contains(&abs_str) {
                continue;
            }
