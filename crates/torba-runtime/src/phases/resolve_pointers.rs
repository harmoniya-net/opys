...
async fn fetch_descriptor(url: &str) -> Result<torba_core::PointerDescriptor, InstallError> {
...
) -> Result<(Source, Option<torba_core::Integrity>, Option<u64>), InstallError> {
    let mut current = source;
    let mut integrity: Option<torba_core::Integrity> = None;
