//! A throwaway HTTP server for the fetch tests.
//!
//! The fetchers talk to a real socket rather than to a mocked transport:
//! `opys-dev`'s HTTP is a plain `ureq` call by design, so this is what keeps
//! the URLs the resolver actually builds — the Meta loader list, the profile,
//! and the Mojang chain the profile inherits from — under test.
//!
