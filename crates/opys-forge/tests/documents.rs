//! Every published Forge document, parsed.
//!
...
//!
//! ```text
//! OPYS_FORGE_DOCUMENTS=/path/to/versions \
//!   cargo test -p opys-forge --test documents -- --ignored --nocapture
//! ```
//!
//! What it is for: the documents are generated once and published, so a
//! parser that reads all 5000-odd of them today reads every Forge build there
//! has ever been. That is a stronger statement than any fixture, and it is
//! the check to re-run when either side changes — the generator's output or
//! `VersionPatch`.
...
fn every_published_document_parses() {
    let root = std::env::var("OPYS_FORGE_DOCUMENTS")
        .expect("set OPYS_FORGE_DOCUMENTS to a directory of published documents");
