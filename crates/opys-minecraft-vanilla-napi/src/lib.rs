//! napi-rs bindings for `opys-minecraft-vanilla`.
//!
//! One addon per crate, named after it: every loader gets its own binding
//! package beside its own crate (`opys-fabric-napi` and the rest to follow),
//! because a binding is packaging and a package that names its own module is
//! what keeps the crate split legible. Collapsing addons is a separate
//! decision, taken for all of them at once rather than crate by crate.
//!
