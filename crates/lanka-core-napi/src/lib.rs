//! napi-rs bindings for `lanka-core`.
//!
...
//! `@lanka/core` index is a thin re-export of these bindings + .d.ts.
...
pub fn decode_manifest(wire: Json) -> Result<Json> {
    let parsed: lanka_core::ManifestWire = serde_json::from_value(wire).map_err(map_err)?;
    let m = lanka_core::decode_manifest(parsed).map_err(map_err)?;
    Ok(lanka_core::encode_manifest(&m))
}
...
pub fn encode_manifest(domain: Json) -> Result<Json> {
    ...
    let wire: lanka_core::ManifestWire = serde_json::from_value(domain).map_err(map_err)?;
    let m = lanka_core::decode_manifest(wire).map_err(map_err)?;
    Ok(lanka_core::encode_manifest(&m))
}
...
pub fn parse_manifest(input: String) -> Result<Json> {
    let m = lanka_core::parse_manifest(&input).map_err(map_err)?;
    Ok(lanka_core::encode_manifest(&m))
}
...
impl From<OsOptionsJs> for lanka_core::OsOptions {
    fn from(o: OsOptionsJs) -> Self {
        lanka_core::OsOptions {
            name: o.name,
            version: o.version,
            arch: o.arch,
        }
...
pub fn resolve_vars(vars: HashMap<String, String>) -> Result<HashMap<String, String>> {
    ...
    let resolved = lanka_core::resolve_vars(&m).map_err(napi::Error::from_reason)?;
...
pub fn interpolate(template: String, vars: HashMap<String, String>) -> String {
    let m: indexmap::IndexMap<String, String> = vars.into_iter().collect();
    lanka_core::interpolate(&template, &m)
}
...
pub fn filter_manifest(manifest: Json, platform: OsOptionsJs, features: Vec<String>) -> Result<Json> {
    let wire: lanka_core::ManifestWire = serde_json::from_value(manifest).map_err(map_err)?;
    let m = lanka_core::decode_manifest(wire).map_err(map_err)?;
    let filtered = lanka_core::filter_manifest(&m, &platform.into(), &features).map_err(map_err)?;
    Ok(lanka_core::encode_manifest(&filtered))
}
...
pub fn resolved_args(launch: Json, platform: OsOptionsJs, features: Vec<String>) -> Result<Vec<String>> {
    let wire: lanka_core::LaunchWire = serde_json::from_value(launch).map_err(map_err)?;
    let l = lanka_core::decode_launch(wire).map_err(map_err)?;
    lanka_core::resolved_args(&l, &platform.into(), &features).map_err(map_err)
}
...
pub fn resolved_envs(
    ...
    let wire: lanka_core::LaunchWire = serde_json::from_value(launch).map_err(map_err)?;
    let l = lanka_core::decode_launch(wire).map_err(map_err)?;
    let env = lanka_core::resolved_envs(&l, &platform.into(), &features).map_err(map_err)?;
...
pub fn satisfies_ruleset(rules: Json, platform: OsOptionsJs, features: Vec<String>) -> Result<bool> {
    let raw: lanka_core::RawRuleset = serde_json::from_value(rules).map_err(map_err)?;
    let parsed = lanka_core::parse_short_ruleset(raw).map_err(map_err)?;
    lanka_mojang_rules::satisfies_ruleset(&parsed, &platform.into(), &features).map_err(map_err)
}

/// Compile a glob to its regex source.
#[napi(js_name = "globToRegexSource")]
pub fn glob_to_regex_source(glob: String) -> String {
    lanka_core::glob_to_regex(&glob).as_str().to_owned()
}

/// The non-wildcard prefix of a glob, used as a sweep starting point.
#[napi(js_name = "globBase")]
pub fn glob_base(glob: String) -> String {
    lanka_core::glob_base(&glob)
}

/// Single discriminated error shape (Q10 in design doc).
#[napi(object, js_name = "LankaErrorInfo")]
pub struct LankaErrorInfo {
    pub code: String,
    pub message: String,
}
