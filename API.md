...
torba builds and runs Minecraft installations from a declarative `torba.json`
manifest. The build side is a **plugin engine**; the runtime side is a dumb
manifest executor. The two are joined only by the frozen `torba.json` format.

## Lifecycle

### `torba build [-i config] [-o out] [--mode m]`

1. `import(config)` — load `torba.config.mjs`.
2. `resolveConfig(default, { mode })` — invoke the function form, if any.
3. `buildManifest(config, ctx)` (`@torba/dev`):
...
2. Read `torba.json` **from disk** (`config.output`) — launch never rebuilds.
3. Apply the `runClient` patch: `{ ...manifest, ...runClient(manifest) }`.
4. `install(manifest)` then `launch(manifest, { install: false })` (`@torba/runtime`).
...
- **`runClient`** — a launch-time manifest patch, re-run every `torba launch`.
...
- **`mode`** — `torba build --mode <m>` → the config function's `ctx.mode`.

## Plugin model — `@torba/dev`
...
`core` is the reference implementation of the `torba.json` format. Every
...
- The `@torba/mojang-rules` rule surface is re-exported from `core`.
...
A `discovery` block on a `url` artifact tells torba how to read `integrity` /
